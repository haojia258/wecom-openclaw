...

{
  ...
  // 验证旧命令仍可用
  const r8 = resolve('/帮助');
  assert(r8 !== null, '旧命令 /帮助 仍然可用');

  const r9 = resolve('/状态');
  assert(r9 !== null, '旧命令 /状态 仍然可用');

  const r10 = resolve('/今日GMV');
  assert(r10 !== null, '旧命令 /今日GMV 仍然可用');
}

// ========== 测试2: commander-policy.js ==========
